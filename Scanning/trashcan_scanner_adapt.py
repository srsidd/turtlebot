...
class GoForward():
	...
	def laser_callback(self, scan):
		...
		mystring = str(sumscore)+'/'+str(self.thresh)
		updatethresh = .90
		self.thresh = updatethresh*self.thresh+(1-updatethresh)*sumscore
		if sumscore>self.thresh:
			# update model
			updaterate = .99
			newim = (updaterate*i0+(1-updaterate)*i2)*255
